...
def check_low_information_score(x):
    ...
    if x[id] >= SCORE_DICT["low_information_score"]+0.05:return ABSTAIN
    elif x[id] <= SCORE_DICT["low_information_score"]-0.05:return FILTERED
    return ABSTAIN

@labeling_function()
def check_odd_aspect_ratio_score(x):
    id = OPS_LIST.index("odd_aspect_ratio_score")
    if x[id] >= SCORE_DICT["odd_aspect_ratio_score"]+0.0:return ABSTAIN
    elif x[id] <= SCORE_DICT["odd_aspect_ratio_score"]-0.0:return FILTERED
    return ABSTAIN

@labeling_function()
def check_odd_size_score(x):
    id = OPS_LIST.index("odd_size_score")
    if x[id] >= SCORE_DICT["odd_size_score"]+0.05:return ABSTAIN
    elif x[id] <= SCORE_DICT["odd_size_score"]-0.05:return FILTERED
    return ABSTAIN

@labeling_function()
def check_image_text_similarity(x):
    id = OPS_LIST.index("image_text_similarity")
    if x[id] >= SCORE_DICT["image_text_similarity"]+0.02:return SELETED
    elif x[id] <= SCORE_DICT["image_text_similarity"]-0.02:return FILTERED
    return ABSTAIN

@labeling_function()
def check_char_rep_ratio(x):
    id = OPS_LIST.index("char_rep_ratio")
    if x[id] >= SCORE_DICT["char_rep_ratio"]+0.02:return FILTERED
    elif x[id] <= SCORE_DICT["char_rep_ratio"]-0.02:return ABSTAIN
    return ABSTAIN
...
def check_lang_and_lang_score(x):
    ...
    return ABSTAIN if x[id1] == SCORE_DICT["lang"] and x[id2]>SCORE_DICT["lang_score"] else FILTERED

@labeling_function()
def check_word_rep_ratio(x):
    id = OPS_LIST.index("word_rep_ratio")
    if x[id] >= SCORE_DICT["word_rep_ratio"]+0.05:return FILTERED
    elif x[id] <= SCORE_DICT["word_rep_ratio"]-0.05:return ABSTAIN
    return ABSTAIN

@labeling_function()
def check_hclip(x):
    id = OPS_LIST.index("hclip_image_text_similarity")
    if x[id] >= SCORE_DICT["hclip_image_text_similarity"]+0.02:return SELETED
    elif x[id] <= SCORE_DICT["hclip_image_text_similarity"]-0.02:return FILTERED
    return ABSTAIN

@labeling_function() 
def check_vclip(x):
    id = OPS_LIST.index("vclip_image_text_similarity")
    if x[id] >= SCORE_DICT["vclip_image_text_similarity"]+0.02:return SELETED
    elif x[id] <= SCORE_DICT["vclip_image_text_similarity"]-0.02:return FILTERED
    return ABSTAIN

@labeling_function()
def check_avg_ratio(x):
    id = OPS_LIST.index("avg_ratio")
    if x[id] > 0.05 and x[id] < SCORE_DICT["avg_ratio"] :return ABSTAIN
    else: return FILTERED

@labeling_function()
def check_num_detections(x):
    id = OPS_LIST.index("num_detections")
    if x[id] >= 1 and x[id] <= SCORE_DICT["num_detections"] :return SELETED
    if x[id] > SCORE_DICT["num_detections"]: return ABSTAIN
    else: return FILTERED

@labeling_function()
def check_avg_score(x):
    id = OPS_LIST.index("avg_score")
    if x[id] >= SCORE_DICT["avg_score"] + 0.01 :return SELETED 
    elif x[id] <= SCORE_DICT["avg_score"] - 0.01 : return FILTERED
    else: return ABSTAIN

@labeling_function()
def check_max_score(x):
    id = OPS_LIST.index("max_score")
    if x[id] >= SCORE_DICT["max_score"] + 0.2:return SELETED
    elif x[id] <= SCORE_DICT["max_score"] - 0.2: return FILTERED
    else: return ABSTAIN

@labeling_function()
def check_icc_score(x):
    id = OPS_LIST.index("icc_score")
    if x[id] >= SCORE_DICT["icc_score"] + 0.00 :return SELETED
    elif x[id] <= SCORE_DICT["icc_score"] - 0.03 : return FILTERED
    else: return ABSTAIN

@labeling_function()
def check_gdino_v1(x):
    id = OPS_LIST.index("scores")
    count = sum(1 for item in x[id] if item > 0.3)
    if count >= 5: 
        return SELETED
    elif count >= 1: 
        return ABSTAIN
...
def check_gdino_v2(x):
    ...
    indices = [index for index, value in enumerate(x[id_scores]) if value > 0.3]
